...
if [ "$1" = '/opt/couchdb/bin/couchdb' ]; then
	...
	if [ ! -z "$NODENAME" ] && ! grep "sname" /opt/couchdb/etc/vm.args; then
		echo "-sname $NODENAME" >> /opt/couchdb/etc/vm.args
	fi

	if [ ! -z "$COOKIE" ] && ! grep "setcookie" /opt/couchdb/etc/vm.args; then
